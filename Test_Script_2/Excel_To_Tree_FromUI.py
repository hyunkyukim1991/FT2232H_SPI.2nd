...
class RegisterTreeViewerFromUI(QMainWindow):
    ...
    def load_ui(self):
        """UI 파일을 로드합니다."""
        import os
        ui_file_path = os.path.join(os.path.dirname(__file__), "register_tree_viewer.ui")
        ...
        try:
            ...
            print(f"UI 로드 중 오류 발생: {e}")
            ui_file.close()
            self.create_fallback_ui()

    def create_fallback_ui(self):
        """UI 파일 로드에 실패했을 때 프로그래밍 방식으로 UI 생성"""
        from PySide6.QtWidgets import (
            QWidget, QVBoxLayout, QHBoxLayout, QSplitter, 
            QTreeWidget, QTextEdit, QLabel, QMenuBar, QStatusBar, QMenu
        )
        from PySide6.QtCore import Qt
        ...
        # 트리 위젯
        self.ui = type('UI', (), {})()  # 빈 객체 생성
        self.ui.tree_widget = QTreeWidget()
        self.ui.tree_widget.setHeaderLabels(["Register/Field"])
        splitter.addWidget(self.ui.tree_widget)

        # 디스크립션 영역
        desc_widget = QWidget()
        desc_layout = QVBoxLayout(desc_widget)
        self.ui.desc_label = QLabel("Description:")
        self.ui.desc_label.setStyleSheet("font-weight: bold;")
        desc_layout.addWidget(self.ui.desc_label)
        
        self.ui.desc_text = QTextEdit()
        self.ui.desc_text.setReadOnly(True)
        self.ui.desc_text.setMaximumHeight(200)
        self.ui.desc_text.setPlaceholderText("Select a register or field to view description")
        desc_layout.addWidget(self.ui.desc_text)
        ...
        self.ui.action_open_excel = QAction("Open Excel File", self)
        self.ui.action_open_excel.setShortcut("Ctrl+O")
        file_menu.addAction(self.ui.action_open_excel)
        
        self.ui.action_save_json = QAction("Save as JSON", self)
        self.ui.action_save_json.setShortcut("Ctrl+S")
        file_menu.addAction(self.ui.action_save_json)
        
        file_menu.addSeparator()
        
        self.ui.action_exit = QAction("Exit", self)
        self.ui.action_exit.setShortcut("Ctrl+Q")
        file_menu.addAction(self.ui.action_exit)
        
        # View 메뉴
        view_menu = menubar.addMenu("View")
        self.ui.action_expand_all = QAction("Expand All", self)
        self.ui.action_expand_all.setShortcut("Ctrl+E")
        view_menu.addAction(self.ui.action_expand_all)
        
        self.ui.action_collapse_all = QAction("Collapse All", self)
        self.ui.action_collapse_all.setShortcut("Ctrl+C")
        view_menu.addAction(self.ui.action_collapse_all)
        
        # Help 메뉴
        help_menu = menubar.addMenu("Help")
        self.ui.action_about = QAction("About", self)
        help_menu.addAction(self.ui.action_about)
    ...
    def load_excel_file(self, excel_path):
        """Excel 파일을 로드하고 트리를 구성합니다."""
        try:
            self.data = self.load_excel(excel_path)
            self.statusBar().showMessage(f"Excel 파일 로드 성공: {excel_path}")
            
            if self.data:
                self.build_tree(self.data)
                
                # JSON 파일로 저장
                json_path = excel_path.replace(".xlsx", "_tree.json").replace(".xls", "_tree.json")
                with open(json_path, "w", encoding="utf-8") as f:
                    json.dump(self.data, f, indent=4, ensure_ascii=False)
                print(f"✅ JSON 파일 저장 완료: {json_path}")
            else:
                self.statusBar().showMessage("데이터를 로드할 수 없습니다.")
                
        except Exception as e:
            QMessageBox.critical(self, "오류", f"Excel 파일 로드 실패:\n{str(e)}")
            self.statusBar().showMessage("Excel 파일 로드 실패")
    ...
    def save_json_file(self):
        ...
        if file_path:
            try:
                ...
                self.statusBar().showMessage(f"JSON 파일 저장 완료: {file_path}")
            except Exception as e:
                QMessageBox.critical(self, "오류", f"JSON 파일 저장 실패:\n{str(e)}")

    def show_about(self):
        """About 다이얼로그를 표시합니다."""
        QMessageBox.about(self, "About", 
                         "Register Tree Viewer\n\n"
                         "Excel 파일에서 레지스터 정보를 읽어\n"
                         "트리 형태로 표시하는 도구입니다.")
    ...
    def on_item_clicked(self, item, column):
        ...
        for reg_addr, reg_data in self.data["Device"].items():
            if item_text.startswith(f"[{reg_addr}]"):
            ...
            if parent:
                ...
                for reg_addr, reg_data in self.data["Device"].items():
                    if parent_text.startswith(f"[{reg_addr}]"):
                        # 필드명 추출 (= 앞부분)
        ...
        self.ui.desc_text.setText(description)
    ...
    def build_tree(self, data):
        ...
        self.ui.tree_widget.addTopLevelItem(device_node)
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_path):
        window = RegisterTreeViewerFromUI(file_path)
        window.show()
        sys.exit(app.exec())
    else:
        print(f"❌ 파일을 찾을 수 없습니다: {file_path}")
        # 파일이 없으면 빈 윈도우로 시작
        window = RegisterTreeViewerFromUI()
        window.show()
        sys.exit(app.exec())